...
int main()
{
    ...
    freopen("./database_final.txt", "r", stdin);
    float ID, C, L, Vp;
    ...
    while (true)
    {
        ...
        float ans [4] = {2, -1, -1, -1};         // all -1 array
        for(int i=0; i<database.size(); i++)
        {
            if (database[i][0] == num[1])       // Search in the database
            {
                for (int j=1; j<4; j++)
        ...
        printf("Send link %.0f, capacity %.0f Mbps, link length %.0f km, and propagation velocity %.0f km/s.\n",
                num[1], ans[1], ans[2], ans[3]);
    }
