 ...
 */
void connectMainServer(int db_socket, std::vector< int * > database)
{
    ssize_t numbytes = 0;
    ...
    int message_in[10];
    if ((numbytes = recvfrom(db_socket, message_in, sizeof(message_in), 0, (struct sockaddr*)other_addr, &addr_len)) == -1)
    {
    ...
    if((numbytes = sendto(db_socket, &ans, sizeof(ans), 0, (struct sockaddr*)other_addr, sizeof(other_addr))) == -1)
    {
        perror("fail sendto");
        exit(1);
    }
    ...
    printf("Receive link %d, capacity %d Mbps, link length %d km, and propagation velocity %d km/s.\n",
            message_in[0], ans[0], ans[1], ans[2]);
}


int main(int argc, char * argv[])
{
    socket_addrinfo dbServer = buildBindSocket(HOSTIP, DBPORT, true);           // create and bind socket according to hostinfo
...
//    kill_zombie_process();          // kill other zombie processes that is currently using this port
    ...
    {
        connectMainServer(dbServer.Socket, database);
    }
