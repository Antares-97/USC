...
class sportsNews extends Component {
    ...
    componentDidMount(){
        ...
        const url = 'https://cs571-hw8-2270250947.appspot.com/api/' + (this.props.isOn? 'guardian/section/sport': 'nytimes/section/sports');
        axios.get(url)
        .then(resp => {
    ...
    loadTopNews(source){
        const url = 'https://cs571-hw8-2270250947.appspot.com/api/' + (source? 'guardian/section/sport': 'nytimes/section/sports');
        axios.get(url)
        .then(resp => {
            this.setState({results: resp.data.results}); 
        })
