...
class politicspage extends Component {
    ...
    componentDidMount(){
        ...
        const url = 'https://cs571-hw8-2270250947.appspot.com/api/' + (this.props.isOn? 'guardian': 'nytimes') + '/section/politics';
        axios.get(url)
        .then(resp => {
    ...
    loadTopNews(source){
        const url = 'https://cs571-hw8-2270250947.appspot.com/api/' + (source? 'guardian': 'nytimes') + '/section/politics';
        axios.get(url)
        .then(resp => {
            this.setState({results: resp.data.results}); 
        })
