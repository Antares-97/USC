...
//  cs571_hw9
//
//  Created by LJZ's macbook pro on 5/5/20.
...
class searchResultsVC: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        delay(seconds: 0.1) {
            SwiftSpinner.show(duration: 2.5, title: "Loading Search Results..")
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "searchResultCell", for: indexPath) as! NewsTableViewCell
    ...
    @IBAction func bookmarkBtn(_ sender: bookmarkButton) {
        let toastMessage = sender.isSelected ? "Article removed from the Bookmarks" : "Article bookmarked. Check out the Bookmarks tab to view"
        self.navigationController?.view!.makeToast(toastMessage, duration: 2.0, position: .bottom)
    }
    ...
    func fetchSearchResults() {
        let query = self.keyword.replacingOccurrences(of: " ", with: "%20")
        
        let urlToSearch = "https://cs571-hw9-2270250947.wm.r.appspot.com/api/guardian/query/?keyword=\(query)"
